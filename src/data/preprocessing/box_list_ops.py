...
from data.preprocessing import shape_utils
...
def scale(boxlist, y_scale, x_scale, scope=None):
    ...
    Returns:
      boxlist: BoxList holding N boxes
    """
    with tf.name_scope('Scale' if scope is None else scope):
...
def prune_completely_outside_window(boxlist, window, scope=None):
    ...
    Returns:
      ...
      valid_indices: a tensor with shape [M_out] indexing the valid bounding boxes
       in the input tensor.
    """
    with tf.name_scope('PruneCompleteleyOutsideWindow' if scope is None else scope):
        y_min, x_min, y_max, x_max = tf.split(
            value=boxlist.get(), num_or_size_splits=4, axis=1)
        win_y_min, win_x_min, win_y_max, win_x_max = tf.unstack(window)
        coordinate_violations = tf.concat([
...
def intersection(boxlist1, boxlist2, scope=None):
    ...
    """
    with tf.name_scope('Intersection' if scope is None else scope):
...
def matched_intersection(boxlist1, boxlist2, scope=None):
    ...
    """
    with tf.name_scope('MatchedIntersection' if scope is None else scope):
        y_min1, x_min1, y_max1, x_max1 = tf.split(
            value=boxlist1.get(), num_or_size_splits=4, axis=1)
        y_min2, x_min2, y_max2, x_max2 = tf.split(
            value=boxlist2.get(), num_or_size_splits=4, axis=1)
...
def iou(boxlist1, boxlist2, scope=None):
    ...
    """
    with tf.name_scope('IOU' if scope is None else scope):
...
def matched_iou(boxlist1, boxlist2, scope=None):
    ...
    """
    with tf.name_scope('MatchedIOU' if scope is None else scope):
...
def ioa(boxlist1, boxlist2, scope=None):
    ...
    """
    with tf.name_scope('IOA' if scope is None else scope):
        intersections = intersection(boxlist1, boxlist2)
...
def change_coordinate_frame(boxlist, window, scope=None):
    ...
    """
    with tf.name_scope('ChangeCoordinateFrame' if scope is None else scope):
