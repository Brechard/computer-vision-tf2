...
from models.yolo.yolo_layers import *
...
class Yolov3:
    ...
    def create_models(self, use_plot_model: bool):
        ...
        if use_plot_model:
            plot_model(train_model, to_file=self.figs_path + 'train_model.png', show_shapes=True, show_layer_names=True)
            plot_model(inference_model, to_file=self.figs_path + 'inference_model.png', show_shapes=True,
    ...
    def tiny_yolo_v3(self, model_input, use_plot_model):
        ...
        from tensorflow.keras.utils import plot_model
        plot_model(train_model, to_file='model.png', expand_nested=True, show_shapes=True)
    ...
    def load_original_yolov3(self, dataset, for_training):
        """
        ...
        if not exists(original_weights_path + '.index'):
            process_original_weights(self.train_model, original_weights_path, self.tiny)
        self.train_model.load_weights(original_weights_path)
        self.inference_model.load_weights(original_weights_path)


def process_original_weights(model, original_weights_path, tiny):
    ...
    layers_model = [l.name for l in model.layers[1:]]
    if tiny:
        layers_model = ['Tiny_DarkNet', 'tiny_layer', 'last_layers_512', 'Concatenate_128', 'last_layers_256']
    for i, layer_name in enumerate(layers_model):
        ...
        if type(sub_model) == tf.keras.Model:
            for i_in, layer in enumerate(sub_model.layers):
                ...
                if not layer.name.startswith('conv2d'):
                    continue
                load_weight(i_in, layer, sub_model, wf)
        elif sub_model.name.startswith('conv2d'):
            load_weight(i, sub_model, model, wf)

    assert len(wf.read()) == 0, 'failed to read all data'
    wf.close()
    model.save_weights(original_weights_path)


def load_weight(pos, layer, sub_model, wf):
